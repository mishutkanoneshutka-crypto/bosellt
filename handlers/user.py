...
async def search_products_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text('Введите текст для поиска товара:', reply_markup=cancel_kb())
    await callback.answer()
...
async def search_products_process(message: Message, state: FSMContext):
    ...
    await state.clear()
    if not products:
        await message.answer('Ничего не найдено.', reply_markup=back_to_menu_kb())
        return
    lines = [
        f'{index}. {product.title} — {format_price(product.price)} ₽ (ID: {product.id})'
    ...
    await message.answer('Чтобы открыть товар, нажмите каталог и выберите его из списка.')
